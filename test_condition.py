...
def get_opt():
    ...
    parser.add_argument('--tocg_checkpoint', type=str, default='checkpoints/test/tocg_final.pth', help='tocg checkpoint')
    ...
    for inputs in test_loader.data_loader:
        ...
        for i in range(c_paired.shape[0]):
            grid = make_grid([(c_paired[i].cpu() / 2 + 0.5), (cm_paired[i].cpu()).expand(3, -1, -1),
                             ...
                             nrow=4)
            save_image(grid,
                       os.path.join('./output', opt.tocg_checkpoint.split('/')[-2], opt.tocg_checkpoint.split('/')[-1],
...
def main():
    opt = get_opt()
    opt.cuda = True  # Added line for CUDA
    print(opt)
    ...
    board = SummaryWriter(log_dir=os.path.join(opt.tensorboard_dir, opt.tocg_checkpoint.split('/')[-2],
                                               opt.tocg_checkpoint.split('/')[-1], opt.datamode, opt.datasetting))
