...
class CPDatasetTest(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print(" coming from cp_dataset_test.py getitem in datasettest class")#not updated in last run
    ...
    def __getitem__(self, index):
        ...
        for key in self.c_names:
            ...
            c[key] = Image.open(osp.join(self.data_path, 'cloth', c_name[key])).convert('RGB')
            #c[key] = transforms.Resize(self.fine_width, interpolation=2)(c[key])
            c[key] = transforms.Resize((self.fine_height, self.fine_width), interpolation=2)(
                c[key])  # Resizing to (Height, Width)
            #print(f"Resized cloth image size: {c[key].size}")
            cm[key] = Image.open(osp.join(self.data_path, 'cloth-mask', c_name[key]))
            #cm[key] = transforms.Resize(self.fine_width, interpolation=0)(cm[key])
            cm[key] = transforms.Resize((self.fine_height, self.fine_width), interpolation=0)(
                cm[key])  # Resizing to (Height, Width)
            #print(f"Resized cloth mask size: {cm[key].size}")
        ...
        im_pil = transforms.Resize((self.fine_height, self.fine_width), interpolation=2)(
            im_pil_big)  # Resizing to (Height, Width)
        ...
        im_parse_pil_big = im_parse_pil_big.resize((256, 384), Image.NEAREST)
        #parse_array = np.array(im_parse_pil_big)       ### is this needed here? added after i noticed it was missing
        #im_parse_pil = transforms.Resize(self.fine_width, interpolation=0)(im_parse_pil_big)

        im_parse_pil = transforms.Resize((self.fine_height, self.fine_width), interpolation=0)(
            im_parse_pil_big)  # Resizing to (Height, Width)
        ...
        #image_parse_agnostic = transforms.Resize(self.fine_width, interpolation=0)(image_parse_agnostic)
        image_parse_agnostic = transforms.Resize((self.fine_height, self.fine_width), interpolation=0)(
            image_parse_agnostic)  # Resizing to (Height, Width)
        ...
        # parse cloth & parse cloth mask##########################################
        pcm = new_parse_map[3:4]
        #print(f"PCM size: {pcm.size()}")  # Log the PCM size
        im_c = im * pcm + (1 - pcm)
        ...
        pose_map = transforms.Resize((self.fine_height, self.fine_width), interpolation=2)(
            pose_map)  # Resizing to (Height, Width)
        ...
        densepose_map = transforms.Resize((self.fine_height, self.fine_width), interpolation=2)(
            densepose_map)  # Resizing to (Height, Width)
        ...
        agnostic = transforms.Resize((self.fine_height, self.fine_width), interpolation=2)(
            agnostic)  # Resizing to (Height, Width)
