...
def get_opt():
    ...
    parser.add_argument("--tensorboard_count", type=int, default=500)
...
def train(opt, train_loader, test_loader, test_vis_loader, board, tocg, generator, discriminator, model):
    ...
    scheduler_gen = torch.optim.lr_scheduler.LambdaLR(optimizer_gen, lr_lambda=lambda step: 1.0 -
                                                                                            max(0,
                                                                                                step * 1000 + opt.load_step - opt.keep_step) / float(
        opt.decay_step + 1))
    ...
    for step in tqdm(range(opt.load_step, opt.keep_step + opt.decay_step)):
        ...
        # Check GPU memory before processing a batch
        #if step % 100 == 0:
        #    print(f"Before training step {step}:")
        ...
        # Resize 'im' and 'pcm' to match fine_height and fine_width
        im = F.interpolate(im, size=(opt.fine_height, opt.fine_width), mode='bilinear')
        pcm = F.interpolate(pcm, size=(opt.fine_height, opt.fine_width), mode='bilinear')

        # Debugging: Print shapes to ensure they match
        #print("im shape:", im.shape)
        #print("pcm shape:", pcm.shape)

        # Perform the operation after ensuring sizes are matched
        im_c = im * pcm + (1 - pcm)

        # Check GPU memory after loading data
        #if step % 100 == 0:
        #    print(f"After loading data for step {step}:")
        #    print(torch.cuda.memory_summary())


            # Clear CUDA cache after loading data
        ...
        with torch.no_grad():
            if not opt.GT:
                ...
                flow = F.interpolate(flow_list[-1].permute(0, 3, 1, 2), size=(iH, iW), mode='bilinear').permute(0, 2, 3,
                                                                                                                1)
                flow_norm = torch.cat(
                ...
                if opt.occlusion:
                    ...
                    warped_cloth_paired = warped_cloth_paired * warped_clothmask + torch.ones_like(
                        warped_cloth_paired) * (1 - warped_clothmask)
        ...
        # Ensure all tensors have the same height and width as 'agnostic'
        target_height, target_width = agnostic.shape[2], agnostic.shape[3]

        pose = F.interpolate(pose, size=(target_height, target_width), mode='bilinear')
        warped_cloth_paired = F.interpolate(warped_cloth_paired, size=(target_height, target_width), mode='bilinear')
        ...
        # Clear CUDA cache after generator updates
        # Before clearing cache
        #print("Before clearing cache:")
        #print(torch.cuda.memory_summary())
        ...
        # Check GPU memory after generator updates
        #if step % 100 == 0:
        #    print(f"After training step {step}:")
        #    print(torch.cuda.memory_summary())
        ...
        # Clear CUDA cache after discriminator updates
        # Before clearing cache
        #print("Before clearing cache:")
        #print(torch.cuda.memory_summary())
        ...
        if (step + 1) % opt.tensorboard_count == 0:
            ...
            grid = make_image_grid(
                [(c_paired[0].cpu() / 2 + 0.5), (cm[0].cpu()).expand(3, -1, -1), ((pose.cpu()[0] + 1) / 2),
                 visualize_segmap(parse_agnostic.cpu(), batch=i),
                 (warped_cloth_paired[i].cpu() / 2 + 0.5), (agnostic[i].cpu() / 2 + 0.5), (pose[i].cpu() / 2 + 0.5),
            ...
            with torch.no_grad():
                if not opt.GT:
                    ...
                    N, _, iH, iW = c_paired.shape
                    grid = make_grid(N, iH, iW, opt)
                    flow = F.interpolate(flow_list[-1].permute(0, 3, 1, 2), size=(iH, iW), mode='bilinear').permute(0,
                                                                                                                    2,
                                                                                                                    3,
                                                                                                                    1)
                    flow_norm = torch.cat(
                ...
                for i in range(opt.num_test_visualize):
                    grid = make_image_grid(
                        [(c_paired[i].cpu() / 2 + 0.5), (cm[i].cpu()).expand(3, -1, -1), ((pose.cpu()[i] + 1) / 2),
                         visualize_segmap(parse_agnostic.cpu(), batch=i),
                         (warped_cloth_paired[i].cpu() / 2 + 0.5), (agnostic[i].cpu() / 2 + 0.5),
                         (pose[i].cpu() / 2 + 0.5), visualize_segmap(fake_parse_gauss.cpu(), batch=i),
                         (output[i].cpu() / 2 + 0.5), (im[i].cpu() / 2 + 0.5)],
                        nrow=4)
                ...
                for i in tqdm(range(500)):
                    ...
                    with torch.no_grad():
                        if not opt.GT:
                            ...
                            # warped cloth
                            N, _, iH, iW = c_paired.shape
                            flow = F.interpolate(flow_list[-1].permute(0, 3, 1, 2), size=(iH, iW),
                        ...
                        old_parse = torch.FloatTensor(fake_parse.size(0), 13, opt.fine_height,
        ...
        if (step + 1) % opt.display_count == 0:
            ...
            print(
                "step: %8d, time: %.3f, G_loss: %.4f, G_adv_loss: %.4f, D_loss: %.4f, D_fake_loss: %.4f, D_real_loss: %.4f"
        ...
        if (step + 1) % opt.save_count == 0:
            save_checkpoint(generator.module,
                            os.path.join(opt.checkpoint_dir, opt.name, 'gen_step_%06d.pth' % (step + 1)), opt)
            save_checkpoint(discriminator.module,
                            os.path.join(opt.checkpoint_dir, opt.name, 'dis_step_%06d.pth' % (step + 1)), opt)
...
def main():
    ...
    if not opt.GT:
        ...
        tocg = ConditionGenerator(opt, input1_nc=input1_nc, input2_nc=input2_nc, output_nc=13, ngf=96,
